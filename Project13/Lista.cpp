...
void ordenarRadixx(std::string& nombre) {
    int maxLength = getMaxLength(nombre);

    // Convertir el nombre a un vector de caracteres
    vector<char> nombreVector(nombre.begin(), nombre.end());

    // Ordenar de acuerdo al último carácter, segundo último carácter, etc.
    for (int i = maxLength - 1; i >= 0; i--) {
        // Crear un vector de 26 listas para almacenar los caracteres que comienzan con cada letra
        vector<vector<char>> buckets(26);

        // Distribuir los caracteres en los buckets
        for (const auto& c : nombreVector) {
            int index = (i < nombreVector.size()) ? tolower(c) - 'a' : 0;
            buckets[index].push_back(c);
        }

        // Reensamblar los caracteres en el orden correcto
        nombreVector.clear();
        for (const auto& bucket : buckets) {
            for (const auto& c : bucket) {
                nombreVector.push_back(c);
            }
        }
    }

    // Convertir el vector de caracteres de vuelta a una cadena en mayúsculas
    nombre = string(nombreVector.begin(), nombreVector.end());
    transform(nombre.begin(), nombre.end(), nombre.begin(), ::toupper);
}
/*void Lista::ordenarRadixx() {
    Nodo* actual = cabeza;
    while (actual != nullptr) {
        radixSort(actual->dato.nombre);
        radixSort(actual->dato.apellido);
        actual = actual->siguiente;
